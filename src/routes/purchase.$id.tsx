...
export const Route = createFileRoute("/purchase/$id")({ component: BillDetailPage });
...
function BillDetailPage() {
  ...
  if (!inv) {
    return (
      <div className="flex flex-col h-full items-center justify-center gap-3 text-gray-400">
        ...
        <button onClick={() => navigate({ to: "/purchase" })} className="text-sm text-primary hover:underline">← Back to Purchase</button>
      </div>
    );
  }
  ...
  if (inv.gstEnabled) {
    inv.lineItems.forEach((l) => {
      ...
      gstBreakdown.set(l.gstRate, { taxable: r2(cur.taxable + taxable), cgst: r2(cur.cgst + half), sgst: r2(cur.sgst + half) });
    });
  }
  ...
  return (
    <div className="flex flex-col h-full bg-gray-100">
      <div className="no-print bg-white border-b px-5 py-3 flex items-center justify-between gap-3">
        <button onClick={() => navigate({ to: "/purchase" })}
      ...
      <div className="flex-1 overflow-auto py-6 px-4 flex justify-center bg-gray-100">
        <div className="bg-white w-full max-w-[794px] shadow-lg print:shadow-none print:m-0 print:p-8"
          style={{ minHeight: "1123px", padding: "40px" }}>

          <div className="flex items-start justify-between mb-8 pb-5 border-b-2 border-primary">
            <div>
              <h1 className="text-[26px] font-extrabold tracking-tight" style={{ color: "oklch(0.55 0.22 27)" }}>
                {co?.name || "My Company"}
              </h1>
              {co?.email && <p className="text-xs text-gray-500 mt-0.5">{co.email}</p>}
              {co?.phone && <p className="text-xs text-gray-500">📞 {co.phone}</p>}
              {co?.gstin && <p className="text-xs font-mono font-semibold text-gray-600 mt-1">GSTIN: {co.gstin}</p>}
              {co?.address && <p className="text-xs text-gray-400 mt-1 max-w-[220px] leading-relaxed">{co.address}</p>}
            </div>
            <div className="text-right">
              <p className="text-[10px] font-bold uppercase tracking-[0.15em] text-gray-400 mb-2">Purchase Bill</p>
              <p className="text-[22px] font-bold font-mono text-gray-800">{inv.number}</p>
              <div className="mt-2 text-xs text-gray-500 space-y-0.5">
                <p><span className="text-gray-400">Date: </span><span className="font-medium">{fmtDate(inv.date)}</span></p>
                <p><span className="text-gray-400">Payment: </span><span className="capitalize font-medium">{inv.paymentMode}</span></p>
              </div>
              {isPaid
                ? <div className="mt-3 inline-flex items-center gap-1 text-[11px] font-bold text-emerald-600 bg-emerald-50 border border-emerald-200 px-2.5 py-1 rounded-full"><Check className="h-3 w-3" /> PAID</div>
                : <div className="mt-3 inline-block text-[11px] font-bold text-rose-600 bg-rose-50 border border-rose-200 px-2.5 py-1 rounded-full">PAYABLE</div>}
            </div>
          </div>

          <div className="mb-7 bg-gray-50 rounded-lg p-4 border border-gray-100">
            <p className="text-[10px] font-bold uppercase tracking-[0.12em] text-gray-400 mb-1.5">Supplier / Vendor</p>
          ...
          <div className="flex justify-end mb-7">
            <div className="w-[280px]">
              <TRow label="Subtotal" value={fmtMoney(inv.subtotal)} />
              {inv.discount > 0 && <TRow label="Discount" value={`−${fmtMoney(inv.discount)}`} cls="text-rose-600" />}
              {inv.gstEnabled && Array.from(gstBreakdown).map(([rate, v]) => (
                <div key={rate}>
              ...
              <div className="mt-2 pt-2 border-t border-gray-200 space-y-1">
                ...
                {balance > 0.01 && <TRow label="Balance Payable" value={fmtMoney(balance)} cls="text-rose-600 font-bold text-[13px]" />}
              </div>
            </div>
          </div>
          ...
          <div className="mt-12 pt-4 border-t border-dashed border-gray-200 text-center">
            <p className="text-[10px] text-gray-300 tracking-wide">Computer-generated document · {co?.name}</p>
          </div>
        </div>
      </div>
    </div>
...
function Th({ children, w, left, center }: { children: React.ReactNode; w?: string; left?: boolean; center?: boolean }) {
  return (
    <th style={{ width: w, textAlign: center ? "center" : left ? "left" : "right", padding: "8px 10px", fontSize: "10px", fontWeight: 700, letterSpacing: "0.08em", textTransform: "uppercase", color: "#6b7280", borderBottom: "2px solid #e5e7eb" }}>
      {children}
    </th>
  );
}

function Td({ children, left, center, bold }: { children: React.ReactNode; left?: boolean; center?: boolean; bold?: boolean }) {
  return (
    <td style={{ textAlign: center ? "center" : left ? "left" : "right", padding: "7px 10px", fontWeight: bold ? 600 : 400, fontVariantNumeric: "tabular-nums" }}>
      {children}
    </td>
  );
}
