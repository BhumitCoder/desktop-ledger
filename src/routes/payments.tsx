...
import type { Payment, PaymentMode, Invoice } from "@/types";
import { fmtMoney, fmtDate, today } from "@/lib/format";
import {
  ArrowDownCircle, ArrowUpCircle, Wallet, Trash2, Search, X,
    ...
    <div className="flex flex-col h-full bg-[#f5f6fa]">
      ...
      <div className="grid grid-cols-3 gap-0 bg-white border-b">
        <div className="px-5 py-3.5 border-r border-gray-100">
          <p className="text-[11px] text-gray-400 font-medium uppercase tracking-wide mb-1">Total Received</p>
        ...
        <div className="flex gap-1">
          {(["all", "in", "out"] as Tab[]).map((t) => (
            <button key={t} onClick={() => setTab(t)}
              className={`px-3 py-1.5 text-xs font-semibold rounded-md transition ${tab === t
                ? t === "in" ? "bg-emerald-50 text-emerald-700" : t === "out" ? "bg-rose-50 text-rose-700" : "bg-gray-100 text-gray-700"
      ...
      <div className="flex-1 overflow-auto">
        <table className="w-full text-[13px] border-collapse">
          <thead className="sticky top-0 bg-white border-b z-10">
            <tr>
              {["Date", "Type", "Party", "Linked Invoice / Bill", "Mode", "Reference", "Amount", ""].map((h) => (
                <th key={h} className={`px-4 py-2.5 text-[11px] font-semibold text-gray-500 uppercase tracking-wide border-b border-gray-100 whitespace-nowrap bg-white ${h === "Amount" ? "text-right" : "text-left"}`}>{h}</th>
              ))}
            </tr>
          </thead>
          <tbody>
            {filtered.length === 0 ? (
              ...
              <tr key={r.id} className="border-b border-gray-100 hover:bg-gray-50/60 transition-colors group">
                ...
                <td className="px-4 py-3">
                  {r.type === "in"
                    ? <span className="inline-flex items-center gap-1 text-[11px] font-semibold text-emerald-700 bg-emerald-50 px-2 py-0.5 rounded-full border border-emerald-200"><ArrowDownCircle className="h-3 w-3" /> Received</span>
                    : <span className="inline-flex items-center gap-1 text-[11px] font-semibold text-rose-700 bg-rose-50 px-2 py-0.5 rounded-full border border-rose-200"><ArrowUpCircle className="h-3 w-3" /> Paid Out</span>}
                </td>
                <td className="px-4 py-3 font-medium text-gray-800 max-w-[150px] truncate">{r.partyName}</td>
                <td className="px-4 py-3 font-mono text-xs text-blue-600">{r.ref && r.ref.match(/^(INV|PUR)-/) ? r.ref : <span className="text-gray-400">—</span>}</td>
                <td className="px-4 py-3 text-gray-500 capitalize text-xs">{r.mode}</td>
                <td className="px-4 py-3 font-mono text-xs text-gray-400">{r.ref && !r.ref.match(/^(INV|PUR)-/) ? r.ref : "—"}</td>
          ...
          {filtered.length > 0 && (
            <tfoot className="sticky bottom-0 bg-gray-50 border-t-2 border-gray-200">
              <tr>
                <td colSpan={6} className="px-4 py-3 text-xs font-bold text-gray-500 uppercase">{filtered.length} record{filtered.length !== 1 ? "s" : ""}</td>
      ...
      <ReceivePaymentDialog open={open} onOpenChange={setOpen} type={formType} onSaved={refresh} />
    </div>
  );
}
...
function ReceivePaymentDialog({
  ...
  const [useInvoiceMode, setUseInvoiceMode] = useState(true);
  ...
  useEffect(() => {
    if (open) {
      setPartyQ(""); setSelectedParty(null); setApplyRows([]);
      setDate(today()); setMode("cash"); setRef(""); setManualAmount("");
      setUseInvoiceMode(true);
      setTimeout(() => partyRef.current?.focus(), 60);
    }
  }, [open]);

  // Load invoices/bills when party selected
  useEffect(() => {
    if (!selectedParty) { setApplyRows([]); return; }
    const repo = isIn ? SalesRepo : PurchaseRepo;
    const invoices = repo.all()
      .filter((inv) => inv.partyId === selectedParty.id && r2(inv.total - inv.paid) > 0.01)
      .sort((a, b) => a.date.localeCompare(b.date));
    setApplyRows(invoices.map((inv) => ({
      invoice: inv,
      due: r2(inv.total - inv.paid),
      apply: 0,
      checked: false,
    })));
  }, [selectedParty, isIn]);
  ...
  const selectParty = (p: { id: string; name: string }) => {
    setSelectedParty(p); setPartyQ(p.name); setPartyOpen(false);
  };

  const totalOutstanding = applyRows.reduce((s, r) => s + r.due, 0);
  const totalApplied = useInvoiceMode
    ? r2(applyRows.reduce((s, r) => s + r.apply, 0))
  ...
  const save = () => {
    if (!selectedParty && !partyQ.trim()) { toast.error("Select or enter a party"); return; }
    const amount = totalApplied;
    if (!amount || amount <= 0) { toast.error("Enter or select an amount to pay"); return; }
    ...
    // Apply to invoices — update their paid fields
    const linked: string[] = [];
    if (useInvoiceMode) {
      const repo = isIn ? SalesRepo : PurchaseRepo;
      for (const row of applyRows) {
        if (row.apply > 0) {
          const newPaid = r2(row.invoice.paid + row.apply);
          repo.update(row.invoice.id, { paid: newPaid } as any);
          linked.push(row.invoice.number);
        }
      }
    }
    ...
    const payment: Payment = {
      id: genId(), date, partyId, partyName, type,
      amount: r2(amount), mode,
      ref: linked.length ? linked.join(", ") : (ref.trim() || undefined),
    ...
    if (linked.length) {
      toast.success(`${fmtMoney(amount)} applied to ${linked.length} ${invWord}${linked.length > 1 ? "s" : ""}`);
    } else {
      ...
      <DialogContent className="max-w-2xl max-h-[90vh] overflow-y-auto">
        <DialogHeader>
          <DialogTitle className="flex items-center gap-2 text-base">
            {isIn
              ? <><ArrowDownCircle className="h-5 w-5 text-emerald-600" /> Receive Payment from Customer</>
              : <><ArrowUpCircle className="h-5 w-5 text-rose-600" /> Make Payment to Supplier</>}
          </DialogTitle>
        </DialogHeader>
        ...
        <div className="space-y-4 mt-1">
          ...
          <div className="relative">
            <label className="flex flex-col gap-1 text-[12px]">
              <span className="font-semibold text-gray-600">{isIn ? "Customer (Received From)" : "Supplier (Paid To)"} *</span>
              <input ref={partyRef} value={partyQ}
                onChange={(e) => { setPartyQ(e.target.value); setPartyOpen(true); setPartyIdx(0); setSelectedParty(null); setApplyRows([]); }}
          ...
          {/* Outstanding panel */}
          {selectedParty && (
            <div className={`rounded-lg border-2 p-4 ${isIn ? "border-emerald-200 bg-emerald-50" : "border-rose-200 bg-rose-50"}`}>
              <div className="flex items-center justify-between mb-3">
                <div>
                  <p className="text-xs font-semibold text-gray-500 uppercase tracking-wide">
                    {isIn ? "Outstanding Receivable from" : "Outstanding Payable to"} {selectedParty.name}
                  </p>
                  <p className={`text-[22px] font-bold tabular-nums mt-0.5 ${isIn ? "text-emerald-700" : "text-rose-700"}`}>
                    {fmtMoney(totalOutstanding)}
                  </p>
                </div>
                {applyRows.length > 0 && (
                  <div className="flex gap-2">
                    <button onClick={applyAll} className="text-xs px-2 py-1 rounded bg-white border font-medium text-gray-700 hover:bg-gray-50">Apply All</button>
                    <button onClick={clearAll} className="text-xs px-2 py-1 rounded bg-white border font-medium text-gray-700 hover:bg-gray-50">Clear</button>
                  </div>
                )}
              </div>
                  ...
                  <div className="bg-white rounded-md border border-gray-200 overflow-hidden max-h-48 overflow-y-auto">
                    {applyRows.map((row, idx) => (
                      <div key={row.invoice.id}
                        ...
                        <button type="button" onClick={() => toggleRow(idx)} className="shrink-0 mt-0.5">
                          {row.checked
                            ? <CheckCircle2 className={`h-5 w-5 ${isIn ? "text-emerald-600" : "text-rose-500"}`} />
                            : <Circle className="h-5 w-5 text-gray-300" />}
                        </button>
                        <div className="flex-1 min-w-0">
                          <div className="flex items-center gap-2">
                            <span className="font-mono font-semibold text-xs text-blue-600">{row.invoice.number}</span>
                            <span className="text-xs text-gray-400">{fmtDate(row.invoice.date)}</span>
                            {row.invoice.paid > 0 && (
                              <span className="text-[10px] font-semibold text-amber-600 bg-amber-50 border border-amber-200 px-1.5 py-0.5 rounded-full">Partial</span>
                            )}
                          </div>
                          <div className="text-xs text-gray-500 mt-0.5">
                            Total {fmtMoney(row.invoice.total)} · Already paid {fmtMoney(row.invoice.paid)}
                          </div>
                        </div>
                        <div className="shrink-0 text-right">
                          <p className="text-[10px] text-gray-400 mb-1">Due</p>
                          <p className={`text-sm font-bold tabular-nums ${isIn ? "text-emerald-700" : "text-rose-700"}`}>{fmtMoney(row.due)}</p>
                        </div>
                        <div className="shrink-0 w-24">
                          <p className="text-[10px] text-gray-400 mb-1 text-right">Apply (₹)</p>
                          <input type="number" value={row.apply || ""} min={0} max={row.due} step="0.01"
              ...
              {applyRows.length === 0 && (
                <div className="mt-3">
                  <label className="text-[12px] font-semibold text-gray-600 block mb-1">Amount (₹) *</label>
                  <input type="number" value={manualAmount} min={0} step="0.01"
                    onWheel={(e) => e.currentTarget.blur()}
          ...
          <div className="grid grid-cols-3 gap-3">
            ...
            <div className="flex flex-col gap-1 text-[12px]">
              ...
              <input value={ref} onChange={(e) => setRef(e.target.value)} placeholder="UPI ref, cheque #…"
                className="h-8 px-2 border rounded bg-white focus:border-primary outline-none text-sm" />
            </div>
          </div>

          {/* Total + Actions */}
          <div className={`rounded-lg border-2 p-4 flex items-center justify-between ${isIn ? "border-emerald-200 bg-emerald-50" : "border-rose-200 bg-rose-50"}`}>
            <div>
              <p className="text-xs text-gray-500 font-medium">
                {useInvoiceMode && applyRows.some((r) => r.checked)
              ...
              <p className={`text-[22px] font-extrabold tabular-nums ${isIn ? "text-emerald-700" : "text-rose-700"}`}>
                {fmtMoney(!selectedParty || applyRows.length === 0 ? (parseFloat(manualAmount) || 0) : totalApplied)}
              </p>
            </div>
            <div className="flex gap-2">
