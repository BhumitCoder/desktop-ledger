...
export const Route = createFileRoute("/purchase-return/$id")({ component: PurchaseReturnDetailPage });
  ...
  if (!ret) {
    return (
      <div className="flex flex-col h-full items-center justify-center gap-3 text-gray-400">
        ...
        <button onClick={() => navigate({ to: "/purchase-return" })} className="text-sm text-primary hover:underline">← Back to Purchase Returns</button>
      </div>
    );
  }
  ...
  return (
    <div className="flex flex-col h-full bg-gray-100">
      <div className="no-print bg-white border-b px-5 py-3 flex items-center justify-between gap-3">
        <button onClick={() => navigate({ to: "/purchase-return" })}
      ...
      <div className="flex-1 overflow-auto py-6 px-4 flex justify-center bg-gray-100">
        <div className="bg-white w-full max-w-[794px] shadow-lg print:shadow-none print:m-0" style={{ minHeight: "1123px", padding: "40px" }}>
          <div className="flex justify-between items-start mb-8">
            <div>
              <h1 className="text-[28px] font-bold text-gray-800 tracking-tight">{co?.name || "Company"}</h1>
            ...
            <div className="text-right">
              <div className="inline-block bg-blue-50 border-2 border-blue-300 rounded-lg px-5 py-3">
                <p className="text-[11px] font-bold text-blue-600 uppercase tracking-wider mb-1">Debit Note</p>
              ...
              <div className="mt-2 text-right text-xs text-gray-500 space-y-0.5">
                <p>Date: <span className="font-semibold text-gray-700">{fmtDate(ret.date)}</span></p>
                {ret.originalRef && <p>Against: <span className="font-semibold text-gray-700 font-mono">{ret.originalRef}</span></p>}
              </div>
            </div>
          </div>

          <div className="border-t-2 border-b border-gray-200 py-4 mb-6">
            <p className="text-[11px] font-semibold text-gray-400 uppercase tracking-wider mb-1">Return To (Supplier)</p>
          ...
          <table className="w-full text-sm mb-6">
            <thead>
              <tr className="bg-gray-50 border-y border-gray-200">
                <th className="text-left px-3 py-2 text-[11px] font-semibold text-gray-500 uppercase tracking-wide">#</th>
                <th className="text-left px-3 py-2 text-[11px] font-semibold text-gray-500 uppercase tracking-wide">Item</th>
                <th className="text-right px-3 py-2 text-[11px] font-semibold text-gray-500 uppercase tracking-wide">Qty</th>
                <th className="text-left px-3 py-2 text-[11px] font-semibold text-gray-500 uppercase tracking-wide">Unit</th>
                ...
                {ret.lineItems.some((l) => l.discountPct > 0) && (
                  <th className="text-right px-3 py-2 text-[11px] font-semibold text-gray-500 uppercase tracking-wide">Disc%</th>
                )}
                {ret.gstEnabled && <th className="text-right px-3 py-2 text-[11px] font-semibold text-gray-500 uppercase tracking-wide">GST%</th>}
              ...
              {ret.lineItems.map((l, i) => (
                <tr key={l.id} className="border-b border-gray-100">
                  ...
                  {ret.lineItems.some((x) => x.discountPct > 0) && (
                    <td className="px-3 py-2.5 text-right text-gray-500">{l.discountPct > 0 ? `${l.discountPct}%` : "—"}</td>
                  )}
                  {ret.gstEnabled && <td className="px-3 py-2.5 text-right text-gray-500">{l.gstRate}%</td>}
                  <td className="px-3 py-2.5 text-right font-semibold tabular-nums">{fmtMoney(l.amount)}</td>
                </tr>
              ))}
            </tbody>
          </table>
          ...
          <div className="flex justify-end mb-6">
            <div className="w-72 space-y-1.5 text-sm">
              <div className="flex justify-between"><span className="text-gray-500">Subtotal</span><span className="tabular-nums">{fmtMoney(ret.subtotal)}</span></div>
                ...
                <>
                  {Array.from(gstBreakdown.entries()).map(([rate, v]) => (
                    <div key={rate} className="text-xs text-gray-400 space-y-0.5">
                      <div className="flex justify-between"><span>CGST ({rate / 2}%)</span><span className="tabular-nums">{fmtMoney(v.cgst)}</span></div>
                      <div className="flex justify-between"><span>SGST ({rate / 2}%)</span><span className="tabular-nums">{fmtMoney(v.sgst)}</span></div>
                    </div>
                  ))}
                  <div className="flex justify-between"><span className="text-gray-500">Total GST</span><span className="tabular-nums">{fmtMoney(ret.taxAmount)}</span></div>
                </>
              )}
          ...
          {ret.notes && (
            <div className="border-t border-gray-200 pt-4 mt-4">
              <p className="text-[11px] font-semibold text-gray-400 uppercase tracking-wider mb-1">Notes / Reason</p>
