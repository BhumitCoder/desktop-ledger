...
function GstPage() {
  ...
  return (
    <div className="flex flex-col h-full">
      <PageHeader title="GST" subtitle={`Output: ${fmtMoney(outTotal)} · Input: ${fmtMoney(inTotal)} · Payable: ${fmtMoney(outTotal - inTotal)}`} />
...
function Section({ title, rows }: { title: string; rows: { rate: number; taxable: number; tax: number }[] }) {
    ...
    <div className="border rounded-md bg-card">
      ...
      <div className="data-table">
        <table className="w-full text-[13px]">
          <thead><tr><th>GST Rate</th><th style={{ textAlign: "right" }}>Taxable Value</th><th style={{ textAlign: "right" }}>Tax Amount</th></tr></thead>
          <tbody>
            {rows.length === 0 ? <tr><td colSpan={3} className="text-center py-6 text-muted-foreground">No entries</td></tr>
              : rows.map((r) => <tr key={r.rate}><td>{r.rate}%</td><td className="text-right">{fmtMoney(r.taxable)}</td><td className="text-right">{fmtMoney(r.tax)}</td></tr>)}
          </tbody>
        </table>
      </div>
      <div className="border-t p-2 flex justify-between font-semibold text-sm"><span>Total Tax</span><span>{fmtMoney(total)}</span></div>
    </div>
  );
}
