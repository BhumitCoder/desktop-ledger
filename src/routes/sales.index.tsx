...
import { SalesRepo, PartyRepo } from "@/repositories";
import type { Invoice } from "@/types";
import { fmtMoney, fmtDate } from "@/lib/format";
import { Plus, Search, X, ChevronDown, FileText, Trash2 } from "lucide-react";
        ...
        <div className="relative">
          ...
          {showPartyDrop && (
            <div className="absolute top-full left-0 mt-1 bg-white border border-gray-200 rounded-lg shadow-lg z-30 w-56 max-h-64 overflow-auto">
              ...
              {parties.length === 0 && <p className="text-xs text-gray-400 text-center py-3">No customers found</p>}
            </div>
          )}
        </div>
          ...
          {filtered.length > 0 && (
            <tfoot className="sticky bottom-0 bg-gray-50 border-t-2 border-gray-200">
              <tr>
                ...
                <td className="px-4 py-3 text-right font-bold text-rose-600 tabular-nums text-sm">{fmtMoney(totalBalance)}</td>
                <td colSpan={3} />
              </tr>
            </tfoot>
          )}
