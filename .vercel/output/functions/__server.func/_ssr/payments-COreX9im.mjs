...
//#region node_modules/.nitro/vite/services/ssr/assets/payments-COreX9im.js
...
function ReceivePaymentDialog({ open, onOpenChange, type, editing, onSaved }) {
	...
	(0, import_react.useEffect)(() => {
		...
		setApplyRows(repo.all().filter((inv) => inv.partyId === selectedParty.id && (r2(inv.total - inv.paid) > .01 || allocOf.has(inv.id))).sort((a, b) => a.date.localeCompare(b.date)).map((inv) => {
